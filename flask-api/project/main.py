...
app = Flask(__name__)
api = Api(app=app, version="0.1", doc="/api", title="Mon API", description="ceci est une description de l'api de test", default="mon api", default_label='ceci est une api de test', validate=True)
# Config Flask Namespace Definition
namespace_user = api.namespace(name='user', description="Fonctions API gestion des utilisateurs", path="/", validate=True)
namespace_users = api.namespace(name='users', description="Fonctions API d'affichage des utilisateurs", path="/", validate=True)
namespace_post = api.namespace(name='article', description="Fonctions API gestion des articles", path="/", validate=True)
namespace_posts = api.namespace(name='articles', description="Fonctions API d'affichage de tous les articles", path="/", validate=True)
...
posts_resource_fields = api.model('ArticleResource', {
    'title': fields.String(description='Le Titre de l\'article', default='Titre par défaut', example='This is a title', required=True),
    'description': fields.String(description='La description de l\'article', default='Description par défaut', example='This is a description', required=True),
...
users_resource_fields = api.model('UserResource', {
    'first_name': fields.String(description='Le Prénom de l\'auteur', default='Prénom', example='Julien', required=True),
    'last_name': fields.String(description='Le Nom de l\'auteur', default='Nom', example='Toutain', required=True),
    'email': fields.String(description='L\'email de l\'auteur', default='Email', example='toutainj@gmail.com', required=True),
    'birthdate': fields.String(description='La date de naissance de l\'auteur', default='Date de naissance', example='2020-01-01', required=True)
})
...
class User(Resource):
    ...
    def post(self):
        ...
        with db.cursor() as cursor:
            ...
            cursor.execute(sql, (api.payload['first_name'], api.payload['last_name'], api.payload['email'], api.payload['birthdate'], author_api_key))
            db.commit()
...
class Articles(Resource):
    ...
    def get(self):
        ...
        if data.get('title'):
            ...
            sql = "SELECT `id`, `title`, `description`, `content`, (SELECT first_name FROM authors WHERE id=author_id) as author, CAST(`date` AS CHAR) as date FROM `posts` \
                    WHERE `author_id`=(SELECT id from authors WHERE first_name LIKE '%"+str(data.get('author_firstname'))+"%');"
        else:
            # sql = "SELECT `id`, `title`, `description`, `content`, (SELECT first_name FROM authors WHERE id=author_id) as author, CAST(`date` AS CHAR) as date FROM `posts`"
    ...
    def post(self):
        ...
        with db.cursor() as cursor:
            ...
            if str(author_key) == str(api.payload['api_key']):
                cursor.execute(sql, (api.payload['title'], api.payload['description'], api.payload['content'], author_id))
                db.commit()
                last_id = cursor.lastrowid
                return last_id
            else:
